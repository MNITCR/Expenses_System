...
const ModalAddExpense = ({showAddModal,setOpenAddModal, currencyCodeRef, reloadCurrencyList}) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.post(`${serverUrl}/${import.meta.env.VITE_API_URL_CURRENCY}`, { code, name, exchange_rate: exchangeRate, userId });
      setCurCode('');
      setCurName('');
  ...
  return (
    <>
      <Modal show={showAddModal} size="md" popup onClose={() => setOpenAddModal(false)} initialFocus={currencyCodeRef}>
        <Modal.Header />
        <Modal.Body>
          <div className="space-y-6">
            <h3 className={`text-xl font-medium text-gray-900 dark:text-white text-center ${i18n.language === 'km' ? "font-kh_siemreap" : ""}`}>{t('add_currency')}</h3>
            <div>
